...
function New-TestKeeper {
    ...
    $cfg = New-TestConfig @{ github = @{ enabled = $true; repoPath = $ClonePath; coordinationBranch = 'coordination'; historyBranch = 'history' } }
...
try {
    ...
    $push = Push-RepoBlobs -RepoPath $repos.clone -Branch 'coordination' `
    ...
    $push2 = Push-RepoBlobs -RepoPath $repos.clone -Branch 'coordination' `
        -Blobs @{ 'coordination/lease.json' = (ConvertTo-Json -InputObject $staleLease -Depth 6) } `
    ...
    $pushX = Push-RepoBlobs -RepoPath $repos.clone -Branch 'coordination' `
    ...
    $pushY = Push-RepoBlobs -RepoPath $repos.clone -Branch 'coordination' `
    ...
    $cfgBad = New-TestConfig @{ github = @{ enabled = $true; repoPath = Join-Path $ws 'missing-repo' } }
    ...
    $cfgMissing = New-TestConfig @{ github = @{ enabled = $true; repoPath = Join-Path $ws 'plain-dir' } }
    ...
    $cfgLocal = New-TestConfig @{ github = @{ enabled = $false } }
