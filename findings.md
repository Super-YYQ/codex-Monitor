...
最终回归追加发现：Windows PowerShell 5.1 的 .NET Framework `Process` 只有 `Kill()`，原降级路径只杀死 `.cmd` 包装器，后台 PowerShell/mock app-server 会继续运行。以唯一临时脚本路径检查进程表的回归先稳定失败，再以共享 `Stop-ProcessTree` 修复；现代运行时走 `Kill(true)`，5.1 在 Windows 上走隐藏 `taskkill /T /F`。源码与解压候选双运行时测试后均无残留。
